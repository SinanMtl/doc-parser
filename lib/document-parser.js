...
export class DocumentParser {
  ...
  escapeRegExp(str) {
    /*
    // TODO: There is a problem. When replacing comments, it also replace some url-ish strings.
    let commentMatch;
    const commentEscapePattern = /(\/\*([\s\S]*?)\*\/|[^\\]\/\/.+)/g;

    // Mask comments with space char
    while ((commentMatch = commentEscapePattern.exec(str)) !== null) {
      const matched = commentMatch[0];
      for (const item of matched.split('\n')) {
        str = str.replace(item, ''.padStart(item.length, ' '));
      }
    } */
    ...
    while ((regexpMatch = regexPattern.exec(str)) !== null) {
      ...
      try {
        ...
        if (match) {
          ...
          str = str.replace(foundRegex, `"${''.padStart(foundRegex.length - 2, '*')}"`);
        }
      } catch {
        continue;
      }
      ...
      if (text && text.length > 0 && !this.isNonTextualContent(text)) {
        ...
        if (!isInScriptTag && !isInStyleTag) {
          ...
          if (!isJSXExpression && !isVueTemplateExpression) {
            ...
            // Calculate positions for actual text content (skip > and <)
            const textStart = match.index + 1 + contentOffset; // Skip opening >
            const textEnd = match.index + match[0].length - 1 + contentOffset; // Skip closing <
            const textPosition = this.getPositionInfo(
              contentForPosition,
              textStart,
              textEnd

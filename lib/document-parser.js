...
export class DocumentParser {
  ...
  extractJavaScriptStringsExcludingJSX(content, jsxRanges) {
    ...
    allStrings.forEach((match) => {
      ...
      const contextStart = Math.max(0, stringIndex - 50);
      const context = content.substring(
        contextStart,
        stringIndex + str.length + 20
      );
      ...
      // Additional filtering for regex patterns, translation keys, and technical content
      if (!shouldIgnore) {
        ...
        shouldIgnore = isRegexFragment || isTranslationPattern || containsTranslationCall || isTooShort || isSpecialChars;
      }
  ...
  /**
   * Replace regex patterns in the string with asterisks
   */
  escapeRegExp(str) {
    const regexPattern = /(!|=)\s{1,}?(\/(.*)\/+([gimuy]*))/gm;
    let regexpMatch;

    while ((regexpMatch = regexPattern.exec(str)) !== null) {
      ...
      try {
        ...
        if (match) {
          ...
          str = str.replace(foundRegex, `"${''.padStart(foundRegex.length, '*')}"`);
        }
      } catch {
        continue;
      }
      ...
      if (text && text.length > 0 && !this.isNonTextualContent(text)) {
        ...
        if (!isInScriptTag && !isInStyleTag) {
          ...
          if (!isJSXExpression && !isVueTemplateExpression) {
            ...
            const adjustedStart = match.index + 1 + contentOffset;
            const adjustedEnd =
              match.index + match[0].length - 1 + contentOffset;
            const position = this.getPositionInfo(
              contentForPosition,
              adjustedStart,
              adjustedEnd
    ...
    naturalLanguageAttrs.forEach((attrName) => {
      ...
      attrMatches.forEach((match) => {
        ...
        if (
          ...
          // Use original content for position calculation if provided (for JSX), otherwise use current content
          const contentForPosition = originalContent || content;
          const adjustedStart = match.index + contentOffset;
          const adjustedEnd = match.index + fullMatch.length + contentOffset;
          const position = this.getPositionInfo(
            contentForPosition,
            adjustedStart,
            adjustedEnd
